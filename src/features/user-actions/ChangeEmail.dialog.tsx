...
const ChangeEmailDialog: FC<Props> = (props): ReactElement => {
  ...
  const {user, signOut, signOutLoading, verifyBeforeUpdateEmail, verifyBeforeUpdateEmailLoading} = useContext(AppAuthContext);
      ...
      <DialogContent className={'flex flex-col gap-6'}>
        ...
        <div className={'flex h-full w-full flex-col items-center justify-center gap-6'}>
          <div className="flex w-full flex-col items-start justify-start gap-2 overflow-hidden">
            ...
            <span title={user?.email || ''} className="w-full overflow-hidden text-ellipsis whitespace-nowrap font-bold">
              {user?.email || ''}
            </span>
          </div>
